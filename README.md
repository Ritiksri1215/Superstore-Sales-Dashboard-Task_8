# Superstore-Sales-Dashboard-Task_8

Super Store Sales Dashboard – Power BI Project

Objective:

The goal of this project was to create a simple interactive sales dashboard that visualizes key performance metrics such as sales, profit, quantity, and category trends over time and across regions.

🛠 Tools Used

Power BI – for dashboard creation and visualization

Microsoft Excel / CSV Dataset (Superstore_Sales.csv) – for data source

(Optional) Python (Pandas) – for basic data cleaning and preprocessing

Dataset Description:

Dataset Name: Superstore_Sales.csv
Columns Used:

Order Date → Converted to Month-Year format

Region

Category

Sales

Profit

Dashboard Features:

The dashboard includes:

Line Chart: Sales trend over months

Bar Chart: Sales by Region

Donut Chart: Sales by Category

KPI Cards: Total Sales, Profit, Quantity

Filter/Slicer: Allows selection by Region or Category

Insights:

West Region recorded the highest sales performance overall.

Office Supplies contributed the largest share of sales compared to Technology and Furniture.

December showed a spike in profits, indicating strong year-end sales performance.

Cash on Delivery (COD) was the most popular payment mode.
